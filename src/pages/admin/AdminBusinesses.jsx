    ...
    <>
      <Panel className="mb-4">
        <div className="relative">
          ...
          <input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search businesses…" className="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-900/[0.03] border border-slate-900/[0.08] focus:outline-none focus:border-primary/50" />
        </div>
      </Panel>
      ...
      <Panel className="overflow-hidden p-0">
        <table className="w-full text-sm">
          ...
          <tbody>
            {rows.map((b) => (
              <tr key={b.id} className="border-t border-slate-900/[0.06] hover:bg-slate-900/[0.03]">
                <td className="px-5 py-4">
                  <div className="flex items-center gap-3">
                    <div className="size-9 rounded-lg bg-gradient-to-br from-primary to-accent grid place-items-center text-xs font-semibold text-primary-foreground">{b.logo}</div>
                    <div>
                ...
                <td className="px-5 py-4 font-mono"><span className={b.trustScore >= 75 ? "text-success" : b.trustScore >= 50 ? "text-warning" : "text-danger"}>{b.trustScore}</span></td>
